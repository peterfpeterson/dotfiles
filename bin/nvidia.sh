...
if [ $(lsmod | grep 'nvidia' -c) ]; then
    ...
    if [[ "${EVERYTHING}" == "Failed"* ]]; then
	nvidia-smi --query-gpu=utilization.gpu,temperature.gpu,memory.used,memory.free,memory.total --format=noheader,nounits --format=noheader,nounits | head -n 2 | head -c 40
    else
	# in percent
